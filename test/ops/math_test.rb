...
module Tensorflow
  class MathTest < Minitest::Test
    ...
    def test_equal
      x = Tensor.new([2, 4])
      y = Tensor.new(2)
      assert_equal([1, 0], Tf.equal(x, y).value)
      assert_equal([1, 0], Tf::Math.equal(x, y).value)

      x = Tensor.new([2, 4])
      y = Tensor.new([2, 4])
      assert_equal([1, 1], Tf.equal(x, y).value)
      assert_equal([1, 1], Tf::Math.equal(x, y).value)
    end

    def test_logical_xor
      x = Tensor.new([false, false, true, true])
      y = Tensor.new([false, true, false, true])
      assert_equal([0, 1, 1, 0], Tf::Math.logical_xor(x, y).value)
    end
    ...
    def test_reduce_any
      ...
      assert_equal(1, Tf.reduce_any(x).value)
      assert_equal([1, 1], Tf.reduce_any(x, axis: 0).value)
      assert_equal([1, 0], Tf.reduce_any(x, axis: 1).value)
    end
